<template>
	<view v-if="show" class="zx-text" :class="block ? 'block' : 'inline'"
...
export default {
	...
	computed: {
		...
		getText() {
			...
			switch(this.mode){
				case 'price':
				    text = this.formatPrice(text);
				    break;
				case 'name':
				    text = this.formatName(text);
				    break;
				case 'date':
				    text = this.formatTime(text);
				    break;
				case 'mobile':
				    text = this.formatMobile(text);
				    break;
				default:
				    break;
			}
		...
		formatMobile(num) {
		   ...
		   /* let value = ''
		    ...
		    for (let i = 0;i < name.length; i++) {
				if(i>2 && i<name.length-4){
					char += '*'
				}
		    }
		    return name.substr(0, 3) + char + name.substr(-4, 4); */
		},
		// 时间格式化
