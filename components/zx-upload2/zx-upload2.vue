<template>
	<view class="zx-upload__container">
		<view class="zx-upload-box">
			<view class="zx-image-item" :style="{width:width+'rpx',height:height+'rpx'}"
				v-for="(item,index) in imageList" :key="index">
				<image :src="item" class="zx-item-img" :style="{width:width+'rpx',height:height+'rpx'}"
					@tap.stop="previewImage(index)" mode="aspectFill"></image>
				<view v-if="!forbidDel" class="zx-img-del" @tap.stop="delImage(index)"></view>
				<view v-if="statusArr[index]!=1" class="zx-upload-mask">
					<view class="zx-upload-loading" v-if="statusArr[index]==2"></view>
					<text class="zx-tips">{{statusArr[index]==2?'上传中...':'上传失败'}}</text>
					<view class="zx-mask-btn" v-if="statusArr[index]==3" @tap.stop="reUpLoad(index)"
						hover-class="zx-btn-hover" :hover-stay-time="150">重新上传</view>
				</view>
			</view>
			<view v-if="isShowAdd" class="zx-upload-add" :style="{width:width+'rpx',height:height+'rpx'}"
				@tap="chooseImage">
				<view class="zx-upload-icon zx-icon-plus"></view>
			</view>
		</view>
	</view>
</template>
...
<style scoped>
	...
	.zx-upload-loading {
		...
		animation: zx-rotate 0.7s linear infinite;
	}
