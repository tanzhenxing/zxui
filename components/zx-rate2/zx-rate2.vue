<template>
	<view class="zx-rate-box" @touchmove="touchMove">
		<block v-for="(item, index) in numbers" :key="index">
			<view class="zx-icon"
				:class="['zx-relative','zx-icon-collection' + (hollow && (current <= index || (disabled && current <= index + 1)) ? '' : '-fill')]"
				:data-index="index" @tap="handleTap"
				:style="{ fontSize: size + 'px', color: current > index + 1 || (!disabled && current > index) ? active : normal }">
				<view class="zx-icon" :class="['zx-icon-main','zx-icon-collection-fill']" v-if="disabled && current == index + 1"
...
<style scoped>
	...
	.zx-icon-collection-fill:before {
		content: '\e6ea';
	}

	.zx-icon-collection:before {
		content: '\e6eb';
	}
