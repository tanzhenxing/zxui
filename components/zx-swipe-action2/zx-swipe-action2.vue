<template>
	<view class="zx-swipeout-wrap" :style="{ backgroundColor: backgroundColor }">
		<view class="zx-swipeout-item" :class="[isShowBtn ? 'swipe-action-show' : '']" 
			:style="{ transform: 'translate(' + position.pageX + 'px,0)' }">
			<view class="zx-swipeout-content" @touchstart="handlerTouchstart"
			...
			<view class="zx-swipeout-button-right-group" v-if="actions.length > 0" @touchend.stop="loop">
				<view class="zx-swipeout-button-right-item" v-for="(item, index) in actions" :key="index"
			...
			<view class="zx-swipeout-button-right-group" @touchend.stop="loop" @tap="handlerParentButton"
...
<style scoped>
	...
	.zx-swipeout-button-right-group {
		position: absolute;
		right: -100%;
		top: 0;
		height: 100%;
