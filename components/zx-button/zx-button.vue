<template>
	...
	>
		<template v-if="loading">
			<zx-loading-icon :mode="loadingMode" :size="loadingSize * 1.15" :color="loadingColor"></zx-loading-icon>
			<text class="u-button__loading-text" :style="[{ fontSize: textSize + 'px' }]">{{ loadingText || text }}</text>
		</template>
		<template v-else>
			<zx-icon v-if="icon" :name="icon" :color="iconColorCom" :size="textSize * 1.35" :customStyle="{ marginRight: '2px' }"></zx-icon>
			<slot>
	...
	>
		...
		<template v-else>
			<zx-icon v-if="icon" :name="icon" :color="iconColorCom" :size="textSize * 1.35"></zx-icon>
			<text
 ...
 * @tutorial https://zxui.org/components/button
 *
 ...
 * @example <zx-button>月落</zx-button>
 */
let timer,flag;
