<template>
	<view class="zx-container" @touchmove.stop.prevent="stop">
		<view class="zx-image-cropper" @touchend="cutTouchEnd" @touchstart="cutTouchStart" @touchmove="cutTouchMove">
			<view class="zx-content">
				...
				<view class="zx-content-middle" :style="{ height: canvasHeight + 'px' }">
					<view class="zx-bg-transparent"
			...
			<image @load="imageLoad" @error="imageLoad" @touchstart="start" @touchmove="move" @touchend="end" :style="{
				...
				}" class="zx-cropper-image" :class="{'zx-cropper__image-hidden':!imageUrl}" :src="imageUrl" mode="widthFix">
			</image>
		</view>
		<canvas canvas-id="zx-image-cropper" id="zx-image-cropper" :disable-scroll="true"
			:style="{ width: CROPPER_WIDTH * scaleRatio + 'px', height: CROPPER_HEIGHT * scaleRatio + 'px' }"
			class="zx-cropper-canvas"></canvas>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.ctx = uni.createCanvasContext('zx-image-cropper', this);
			this.setCutCenter();
