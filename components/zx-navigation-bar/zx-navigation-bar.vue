<template>
	<view class="zx-navigation-bar"
		:class="{ 'zx-bar-line': opacity > 0.85 && splitLine, 'zx-navbar-fixed': isFixed, 'zx-backdrop__filter': backdropFilter && dropDownOpacity > 0  }"
		:style="{ height: height + 'px', backgroundColor: `rgba(${background},${opacity})`, opacity: dropDownOpacity, zIndex: isFixed ? zIndex : 'auto' }">
		<view class="zx-status-bar" :style="{ height: statusBarHeight + 'px' }" v-if="isImmersive"></view>
		<view class="zx-navigation_bar-title"
