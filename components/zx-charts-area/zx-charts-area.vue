		...
		<view class="zx-charts__line-box" v-if="xAxis.length>0 && dataset.length>0" :style="{width:width+'rpx'}">
			<scroll-view :scroll-x="scrollable" class="zx-line__scroll-view" :style="{height:scrollViewH+'rpx'}">
				...
				<view class="zx-charts__line" :style="{height:height+'rpx'}">
					...
					<view class="zx-area__chart-box" v-for="(area,i) in dots" :key="area.key"
						:style="{left:xGap/2+'rpx',right:xGap/2+'rpx',minWidth:xWidth+'rpx'}">
						<view class="zx-area__item"
							:style="{'--zx-area-start': model.start, '--zx-area-end': model.end,background:model.background}"
...
<style scoped>
	...
	.zx-area__item {
		...
		clip-path: polygon(0% calc(100% * (1 - var(--zx-area-start))),
				100% calc(100% * (1 - var(--zx-area-end))),
				120% 100%,
				-20% 100%);
	}
</style>
