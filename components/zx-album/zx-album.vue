	...
	<view class="zx-album">
		<view class="zx-album__row" ref="zx-album__row" v-for="(arr, index) in showUrls" :forComputedUse="albumWidth" :key="index">
			<view class="zx-album__row__wrapper" v-for="(item, index1) in arr" :key="index1"
				...
				<view v-if="showMore && urls.length > rowCount * showUrls.length && index === showUrls.length - 1 && index1 === showUrls[showUrls.length - 1].length - 1" class="zx-album__row__wrapper__text">
...
export default {
	...
	methods: {
		...
		// 获取图片的路径
		getSrc(item) {
			return this.testObject(item) ? (this.keyName && item[this.keyName]) || item.src : item;
		},
		// 单图时，获取图片的尺寸
		...
		async getComponentWidth() {
			...
			await this.sleep(30);
			// #ifndef APP-NVUE
			this.$uGetRect('.zx-album__row').then(size => {
				this.singleWidth = size.width * this.singlePercent;
			});
			// #endif

			// #ifdef APP-NVUE
			// 这里ref="zx-album__row"所在的标签为通过for循环出来，导致this.$refs['zx-album__row']是一个数组
			const ref = this.$refs['zx-album__row'][0];
			ref &&
