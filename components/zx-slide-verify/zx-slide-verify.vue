<template>
	<view class="zx-slide-vcode" :style="{width:slideBarWidth+'px',height:slideBlockWidth+'px',backgroundColor:backgroundColor}">
		<text class="zx-text-flashover" :style="{fontSize:size+'rpx',background:getBgColor}">拖动滑块验证</text>
		<view class="zx-slide-glided" :style="{backgroundColor:activeBgColor}">
			<text :style="{fontSize:size+'rpx',color:activeColor}" v-if="isPass">{{passText}}</text>
		</view>
		<view class="zx-slider-block" :style="{width:slideBlockWidth+'px',height:slideBlockWidth+'px',borderColor:isPass?activeBorderColor: borderColor}"
		 ...
		 @touchend="parse.touchend">
			<text class="zx-slide-icon zx-icon-double_arrow" :style="{fontSize:iconSize+'rpx',color:arrowColor}" v-if="!isPass"></text>
			<text class="zx-slide-icon zx-icon-check_mark" :style="{fontSize:iconSize+'rpx',color:checkColor}" v-if="isPass"></text>
		</view>
	</view>
</template>
<script src="./zx-slide-verify.wxs" module="parse" lang="wxs"></script>
<script>
	export default {
		name: "zx-slide-verify",
		emits: ['success'],
		props: {
			//滑动条宽度 px
			slideBarWidth: {
...
<style scoped>
	...
	.zx-icon-check_mark:before {
		content: "\e634";
	}

	.zx-icon-double_arrow:before {
		content: "\e600";
	}
