<template>
	<view class="zx-notice" @tap="clickHandler">
		...
		<view class="zx-notice__content" ref="zx-notice__content">
			<view ref="zx-notice__content__text" class="zx-notice__content__text" :style="[animationStyle]">
		...
		<view class="zx-notice__right-icon" v-if="['link', 'closable'].includes(mode)">
			...
			<zx-icon v-if="mode === 'closable'" @click="close" name="close" :size="16" :color="color"></zx-icon>
		</view>
	</view>
</template>
<script>
...
export default {
	...
	methods: {
		...
		async vue() {
			...
			await util.sleep();
			// 查询盒子和文字的宽度
			//textWidth = (await util.getRect('.zx-notice__content__text')).width;
			//boxWidth = (await util.getRect('.zx-notice__content')).width;
			// 根据t=s/v(时间=路程/速度)，这里为何不需要加上#zx-notice-box的宽度，因为中设置了.zx-notice-content样式中设置了padding-left: 100%
			// 恰巧计算出来的结果中已经包含了#zx-notice-box的宽度
			this.animationDuration = `${textWidth / uni.upx2px(this.speed)}s`;
		...
		async nvue() {
			...
			await util.sleep();
			// 查询盒子和文字的宽度
			textWidth = (await this.getNvueRect('zx-notice__content__text')).width;
			boxWidth = (await this.getNvueRect('zx-notice__content')).width;
			// 将文字移动到盒子的右边沿，之所以需要这么做，是因为nvue不支持100%单位，否则可以通过css设置
			animation.transition(
				this.$refs['zx-notice__content__text'],
				{
					styles: {
		...
		loopAnimation(textWidth, boxWidth) {
			...
			animation.transition(
				this.$refs['zx-notice__content__text'],
				{
					...
					duration: ((boxWidth + textWidth) / parseInt(this.speed)) * 1000,
					delay: 10
				},
				() => {
					animation.transition(
...
.zx-notice {
	...
	&__content {
		...
		&__text {
			...
			word-break: keep-all;
			white-space: nowrap;
			animation: zx-loop-animation 10s linear infinite both;
			/* #endif */
			display: flex;
			flex-direction: row;
		}
