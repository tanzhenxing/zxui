	...
	<view class="gui-schedule-wrap">
		<view class="gui-flex gui-row gui-space-between gui-align-items-center">
			<picker mode="date" :value="currentDayIn" :start="startDate" :end="endDate" @change="selectDate">
				<text class="gui-schedule-header-date gui-icons gui-primary-text">{{ cYear }} 年 {{ cMonthStr }} 月 &#xe603;</text>
			</picker>
		...
		<view class="gui-flex gui-row gui-nowrap">
			<text class="gui-schedule-weeks gui-color-gray gui-block" v-for="(item, index) in weeks" :key="index">{{ item }}</text>
		</view>
		<view class="gui-flex gui-row gui-wrap">
			<view class="gui-schedule-7item gui-flex gui-row gui-justify-content-center" v-for="(item, index) in days" :key="index">
				<view
					v-if="item != ''"
					class="gui-date-ditems gui-flex gui-columns gui-justify-content-center"
					:class="[currentDayIn == cYear + '-' + cMonthStr + '-' + item.date ? 'gui-bg-primary' : 'gui-bg-gray gui-dark-bg-level-3']"
					@click="chooseDate(cYear + '-' + cMonthStr + '-' + item.date, item.date)"
				>
					<text class="gui-date-day gui-block" :class="[currentDayIn == cYear + '-' + cMonthStr + '-' + item.date ? 'gui-color-white' : 'gui-primary-text']">
						{{ item.date }}
					</text>
					<text
						class="gui-date-nl gui-block"
						v-if="isLunar"
						:class="[currentDayIn == cYear + '-' + cMonthStr + '-' + item.date ? 'gui-color-white' : 'gui-primary-text']"
					>
						{{ item.nl }}
					</text>
					<view class="gui-schedule-point" v-if="item.haveSe" :style="{ backgroundColor: pointColor }"></view>
				</view>
		...
		<view class="gui-schedule-time-list-wrap">
			<view class="gui-schedule-time-list gui-flex gui-row gui-nowrap" v-for="(item, index) in hours" :key="index">
				<text class="gui-schedule-timer gui-block gui-color-gray">{{ item }}:00</text>
