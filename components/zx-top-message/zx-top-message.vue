<template>
	<view class="gui-top-message" ref="guipopupfortopmsg" v-if="show" :style="{ top: navHeight + 'px' }" :class="[out ? 'gui-top-message-out' : 'gui-top-message-in']">
		<slot></slot>
	</view>
</template>
<script>
...
.gui-top-message-in {
	animation: gui-top-message-in 350ms linear forwards;
}
@keyframes gui-top-message-out {
	0% {
		transform: translateY(0px);
		opacity: 1;
	}
...
.gui-top-message-out {
	animation: gui-top-message-out 350ms linear forwards;
}
/* #endif */
</style>
