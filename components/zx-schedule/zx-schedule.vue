<template>
	<view class="gui-schedule-wrap">
		...
		class="gui-flex gui-row gui-space-between gui-align-items-center">
			...
			@change="selectDate">
				<text 
				class="gui-schedule-header-date gui-icons gui-primary-text">{{cYear}} 年 {{cMonthStr}} 月 &#xe603;</text>
			</picker>
			<text 
			class="gui-border gui-schedule-today gui-secondary-text" 
			@tap="gotoToday">返回今天</text>
		</view>
		<view 
		class="gui-flex gui-row gui-nowrap">
			<text 
			class="gui-schedule-weeks gui-color-gray gui-block" 
		...
		<view class="gui-flex gui-row gui-wrap">
			...
			:key="index">
				...
				:class="[
					currentDayIn == cYear+'-'+cMonthStr+'-'+ item.date ? 'gui-bg-primary' : 'gui-bg-gray gui-dark-bg-level-3'
				]"
				@click="chooseDate(cYear+'-'+cMonthStr+'-'+item.date, item.date)">
					<text 
					class="gui-date-day gui-block" 
					:class="[
						currentDayIn == (cYear+'-'+cMonthStr+'-'+item.date) ? 'gui-color-white' : 'gui-primary-text'
					]">{{item.date}}</text>
					<text 
					class="gui-date-nl gui-block" 
					v-if="isLunar" 
					:class="[
						currentDayIn == (cYear+'-'+cMonthStr+'-'+item.date) ? 'gui-color-white' : 'gui-primary-text']">{{item.nl}}</text>
					<view 
					class="gui-schedule-point" 
					v-if="item.haveSe" 
					:style="{backgroundColor:pointColor}"></view>
				</view>
				<view 
				class="gui-date-ditems" 
				v-else></view>
			</view>
		</view>
		<view class="gui-border-b gui-schedule-line"></view>
		<view 
		class="gui-schedule-time-list-wrap">
			<view 
			class="gui-schedule-time-list gui-flex gui-row gui-nowrap"
			v-for="(item, index) in hours" 
			:key="index">
				<text 
				class="gui-schedule-timer gui-block gui-color-gray">{{item}}:00</text>
				<view 
				class="gui-border-b gui-schedule-body gui-flex gui-columns">
					<text 
					class="gui-schedules gui-block" 
					v-for="(schedule, idx) in schedulesIn[index]" 
					:key="idx" 
					@tap="scheduleTap" 
					:data-id="schedule.id" 
					:style="{
...
export default{
	...
	emits:['scheduleTap', 'selectDate', 'chooseDate', 'gotoToday']
}
</script>
<style scoped>
.gui-schedule-wrap{width:690rpx;}
.gui-schedule-header-date{height:88rpx; line-height:88rpx; font-size:28rpx;}
...
.gui-date-nl{height:24rpx; line-height:26rpx; font-size:20rpx; text-align:center;}
...
.gui-schedule-body{width:200rpx; flex:1; margin-top:15rpx;}
...
.gui-schedule-today{line-height:50rpx; height:50rpx; font-size:22rpx; padding-left:20rpx; padding-right:20rpx;}
