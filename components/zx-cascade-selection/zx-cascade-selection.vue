<template>
	<view class="zx-cascade-selection">
		<scroll-view scroll-x scroll-with-animation :scroll-into-view="scrollViewId"
			:style="{ backgroundColor: headerBgColor }" class="zx-bottom-line"
			:class="{ 'zx-btm-none': !headerLine }">
			<view class="zx-selection-header" :style="{ height: tabsHeight, backgroundColor: backgroundColor }">
				<view class="zx-header-item" :class="{ 'zx-font-bold': idx === currentTab && bold }"
					...
					:key="idx">
					{{ item.text }}
					<view class="zx-active-line" :style="{ backgroundColor: lineColor }"
						v-if="idx === currentTab && showLine"></view>
				</view>
			</view>
		</scroll-view>
		<swiper class="zx-selection-list" :current="defTab" duration="300" @change="switchTab"
			:style="{ height: height, backgroundColor: backgroundColor }">
			<swiper-item v-for="(item, index) in selectedArr" :key="index">
				<scroll-view scroll-y :scroll-into-view="item.scrollViewId" class="zx-selection-item"
					:style="{ height: height }">
					<view class="zx-first-item" :style="{ height: firstItemTop }"></view>
					<view class="zx-selection-cell" :style="{ padding: padding, backgroundColor: backgroundColor }"
						...
						<icon type="success_no_circle" v-if="item.index === subIndex" :color="checkMarkColor"
							:size="checkMarkSize" class="zx-icon-success"></icon>
						<image :src="subItem.src" v-if="subItem.src" class="zx-cell-img"
							:style="{ width: imgWidth, height: imgHeight, borderRadius: radius }"></image>
						<view class="zx-cell-title"
							:class="{ 'zx-font-bold': item.index === subIndex && textBold, 'zx-flex-shrink': nowrap }"
							:style="{ color: item.index === subIndex ? textActiveColor : textColor, fontSize: textSize + 'rpx' }">
							{{ subItem.text }}
						</view>
						<view class="zx-cell-sub_title" :style="{ color: subTextColor, fontSize: subTextSize + 'rpx' }"
