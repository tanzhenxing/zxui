<template>
	<view :style="{ opacity: pageStatus ? 1 : 0 }" class="gui-sbody gui-flex gui-column" :class="[fullPage || refresh || loadmore ? 'gui-flex1' : '']">
		<!-- 自定义头部 -->
		<view class="gui-header gui-transition-all" :class="headerClass" v-if="customHeader" id="guiPageHeader" ref="guiPageHeader">
			<!-- 状态栏 -->
			<view class="gui-page-status-bar" :class="statusBarClass" :style="{ height: statusBarHeight + 'px' }"><slot name="gStatusBar"></slot></view>
			<!-- 头部插槽 -->
			<view class="gui-flex gui-column gui-justify-content-center" @tap.stop.prevnet="headerTap"><slot name="gHeader"></slot></view>
		</view>
		<!-- 自定义头部占位 -->
		<view v-if="customHeader && isHeaderSized" :style="{ height: headerHeight + 'px' }"></view>

		<!-- 页面主体 -->
		<view class="gui-flex gui-column gui-relative" v-if="!refresh && !loadmore" id="guiPageBody" :class="[fullPage ? 'gui-flex1' : '']" ref="guiPageBody">
			<slot name="gBody"></slot>
		</view>

		<!-- 刷新加载主体  非 nvue -->
		<!-- #ifndef APP-NVUE -->
		<view class="gui-flex1 gui-relative" v-if="refresh || loadmore" id="guiPageBody" ref="guiPageBody" :style="{ marginTop: fixedTopMargin + 'px' }">
			<scroll-view
			...
			>
				...
				<view v-if="loadmore">
					<gui-loadmore
						...
						:loadMoreFontSize="loadMoreFontSize"
					></gui-loadmore>
				</view>
			</scroll-view>
		</view>
		...
		<!-- 刷新加载主体 nvue -->
		<!-- #ifdef APP-NVUE -->
		<view class="gui-flex gui-column gui-flex1" v-if="refresh || loadmore" id="guiPageBody" ref="guiPageBody" :style="{ marginTop: fixedTopMargin + 'px' }">
			<list :show-scrollbar="false" class="gui-flex1" @loadmore="loadmorefun" @scroll="scroll">
				<refresh v-if="refresh" :display="refreshing ? 'show' : 'hide'" @refresh="onrefresh" @pullingdown="onpullingdown"></refresh>
		...
		<!-- 页面底部 -->
		<!-- 底部占位 -->
		<view v-if="customFooter" :style="{ height: footerHeight + 'px' }"></view>
		<view v-if="customFooter" class="gui-page-footer gui-border-box" id="guiPageFooter" ref="guiPageFooter" :class="footerClass">
			<slot name="gFooter"></slot>
			<gui-iphone-bottom :need="!isSwitchPage" :customClass="footerSpaceClass"></gui-iphone-bottom>
		</view>

		<!-- 右下角悬浮挂件 -->
		<view class="gui-page-pendant" :class="pendantClass"><slot name="gPendant"></slot></view>

		<!-- 吸顶元素 -->
		<view class="gui-page-fixed-top" ref="guiPageFixedTop" id="guiPageFixedTop" :style="{ top: fixedTop + 'px' }"><slot name="gFixedTop"></slot></view>
