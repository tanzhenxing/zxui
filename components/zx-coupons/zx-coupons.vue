<template>
	<view class="gui-coupons gui-flex gui-row gui-nowrap">
		...
		>
			<view class="gui-coupons-left-number">
				<view class="gui-flex gui-row gui-nowrap gui-justify-content-center gui-align-items-center">
					...
					<text class="gui-h3 gui-bold" :style="{ color: coupon.color }">{{ coupon.number }}</text>
				</view>
				<text class="gui-text-small gui-block gui-text-center" :style="{ color: coupon.color }">{{ coupon.txt }}</text>
			</view>
			...
			<text class="gui-coupons-status" v-if="coupon.drawed" :style="{ backgroundColor: coupon.color }">{{ coupon.drawed }}</text>
		</view>
		<text
		...
		<view
			class="gui-coupons-sawtooth"
			:style="{
				height: coupon.height,
				backgroundColor: coupon.color
			}"
		>
			<view class="gui-coupons-sawtooth-circular gui-bg-gray gui-dark-bg-level-3" v-for="(item, index) in 10" :key="index"></view>
		</view>
	</view>
</template>
<script>
