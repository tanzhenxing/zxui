<template>
	<view @touchmove.stop.prevent>
		<view class="zx-fab-box" :class="{'zx-fab-right':!left || (left && right)}"
			:style="{left:getLeft(),right:getRight(),bottom:bottom+'rpx'}">
			<view class="zx-fab-btn" :class="{'zx-visible':isOpen,'zx-fab-hidden':isHidden}">
				<view class="zx-fab-item-box" :class="{'zx-fab-item-left':left && !right && item.imgUrl}"
					v-for="(item,index) in btnList" :key="index" @tap.stop="handleClick(index)">
					<view :class="[left && !right?'zx-text-left':'zx-text-right']" v-if="item.imgUrl"
						:style="{fontSize:item.fontSize+'rpx',color:item.color}">{{item.text || ""}}</view>
					<view class="zx-fab-item"
						:style="{width:width+'rpx',height:height+'rpx',background:item.bgColor || bgColor,borderRadius:radius}">
						<view class="zx-fab-title" v-if="!item.imgUrl"
							:style="{fontSize:item.fontSize+'rpx',color:item.color}">{{item.text || ""}}</view>
						<image :src="item.imgUrl" class="zx-fab-img" v-else
							:style="{width:item.imgWidth+'rpx',height:item.imgHeight+'rpx'}"></image>
					</view>
				</view>
			</view>
			<view class="zx-fab-item" :class="{'zx-active':isOpen}"
				:style="{width:width+'rpx',height:height+'rpx',borderRadius:radius,background:bgColor,color:color}"
				@tap.stop="handleClick(-1)">
				<text class="zx-fab-icon zx-icon-plus" v-if="!custom"></text>
				<slot></slot>
			</view>
		</view>
		<view class="zx-fab-mask" :class="{'zx-visible':isOpen}" @tap="handleClickCancel"></view>
	</view>
</template>
