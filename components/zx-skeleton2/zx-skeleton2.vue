<template>
	<view class="zx-skeleton-cmomon zx-skeleton-box"
		:style="{width: winWidth+'px', height:winHeight+'px', backgroundColor:backgroundColor}">
		<view class="zx-skeleton-cmomon" :class="{'zx-skeleton__active':active}" v-for="(item,index) in skeletonElements" :key="index"
		...
		<view class="zx-loading" :class="[getLoadingType(loadingType)]" v-if="isLoading"></view>
	</view>
</template>
...
<script>
	export default {
		...
		props: {
			...
			//默认所有，根据页面情况进行传值
			//页面对应元素class为：zx-skeleton-rect，zx-skeleton-circular，zx-skeleton-fillet
...
<style scoped>
	...
	.zx-skeleton__active {
		...
		animation: zx-active 1.4s ease infinite;
