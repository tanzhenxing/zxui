<template>
	<view class="gui-flex gui-rows gui-wrap">
		<view class="gui-add-list-items" :style="{ borderRadius: borderRadius }" v-for="(item, index) in imgLists" :key="index">
			<image :src="item.url" :data-imgurl="item.url" :style="{ borderRadius: borderRadius }" @tap="showImgs" class="gui-add-list-img" :mode="imgMode"></image>
			<!-- 进度条 -->
			...
			<!-- 上传失败 -->
			<view class="gui-add-list-reup gui-flex gui-columns gui-justify-content-center gui-align-items-center" @tap.stop="retry" :data-index="index" v-if="item.error">
			...
			<!-- 删除 -->
			<text class="gui-add-list-remove gui-icons" :style="{ color: removeBtnColor }" @tap="removeImg" :id="'gui-items-img-' + index">&#xe632;</text>
		</view>
		<view
	...
	methods: {
		...
		addImg: function() {
			...
			uni.chooseImage({
				...
				success: res => {
					if (this.imgLists.length >= this.maxFileNumber) {
						return;
					}
...
<style scoped>
.gui-add-list-btn-text {
	font-size: 26rpx;
	line-height: 36rpx;
	text-align: center;
}
.gui-add-list-btn-icon {
	font-size: 80rpx;
	height: 80rpx;
	line-height: 80rpx;
	margin-bottom: 20rpx;
...
.gui-add-list-reup-icon {
	text-align: center;
	font-size: 68rpx;
	line-height: 100rpx;
}
.gui-add-list-reup-text {
	text-align: center;
	font-size: 20rpx;
	line-height: 30rpx;
}
