<template>
	...
	>
		<view class="gui-sbutton" v-if="status == 1"><slot name="default"></slot></view>
		<view class="gui-sbutton gui-flex gui-rows gui-nowrap gui-justify-content-center gui-align-items-center" :class="[baseClass]" v-if="status == 2">
		...
		<view class="gui-sbutton" v-if="status == 3"><slot name="success"></slot></view>
		<view class="gui-sbutton" v-if="status == 4"><slot name="error"></slot></view>
...
export default {
				...
				case 2:
					setTimeout(() => {
						this.getRefs('loadingPoints1', 0, refs => {
							this.BindingXObjs = [refs.ref, this.$refs.loadingPoints2.ref, this.$refs.loadingPoints3.ref];
							this.startAnimate();
						});
					}, 100);
	...
	methods: {
		...
		startAnimate: function() {
			...
			setTimeout(() => {
				this.loadingAnimate(1);
			}, 300);
			setTimeout(() => {
				this.loadingAnimate(2);
			}, 600);
		},
...
/* #ifndef APP-NVUE */
.gui-sbutton-loading1 {
	animation: gui-sbutton-loading1 1200ms ease-in infinite;
}
...
.gui-sbutton-loading2 {
	animation: gui-sbutton-loading2 1200ms ease-in infinite;
}
...
.gui-sbutton-loading3 {
	animation: gui-sbutton-loading3 1200ms ease-in infinite;
}
...
.gui-btn-fade-in {
	animation: gui-btn-fade-in 350ms ease-in forwards;
}
/* #endif */
</style>
