<template>
	<view class="zx-tabs-view"
		:class="[isFixed ? 'zx-tabs-fixed' : 'zx-tabs-relative', unlined ? 'zx-unlined' : '']" :style="{
			width: tabsWidth + 'px',
			height: height + 'rpx',
		...
		<view v-for="(item, index) in tabs" :key="index" class="zx-tabs-item"
			:style="{ width: itemWidth,height: height + 'rpx' }" @tap.stop="swichTabs(index)">
			<view class="zx-tabs-title"
				:class="{ 'zx-tabs-active': currentTab == index, 'zx-tabs-disabled': item.disabled }" :style="{
					color: currentTab == index ? selectedColor : color,
					fontSize: size + 'rpx',
					fontWeight: bold && currentTab == index ? 'bold' : 'normal'
				}">
				{{ item.name }}
				<view :class="[item.isDot ? 'zx-badge__dot' : 'zx-tabs__badge']"
					:style="{ color: badgeColor, backgroundColor: badgeBgColor }" v-if="item.num || item.isDot">
					{{ item.isDot ? '' : item.num }}
				</view>
			</view>
		</view>
		<view v-if="isSlider" class="zx-tabs-slider" :style="{
...
<style scoped>
	...
	.zx-tabs-fixed::before,
	.zx-tabs-relative::before {
		content: '';
		position: absolute;
