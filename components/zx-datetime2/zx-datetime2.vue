<template>
	<view class="zx-datetime-picker">
		<view class="zx-mask" :class="{ 'zx-mask-show': isShow }" @touchmove.stop.prevent="stop" catchtouchmove="stop"
			@tap="maskClick"></view>
		<view class="zx-header" :class="{ 'zx-show': isShow }">
			<view class="zx-picker-header" :class="{ 'zx-date-radius': radius }"
				:style="{ backgroundColor: headerBackground }" @touchmove.stop.prevent="stop" catchtouchmove="stop">
				<view class="zx-btn-picker" :style="{ color: cancelColor }" hover-class="zx-opacity"
					:hover-stay-time="150" @tap="hide">取消</view>
				<view class="zx-pickerdate__title" :style="{fontSize:titleSize+'rpx',color:titleColor}">{{title}}
				</view>
				<view class="zx-btn-picker" :style="{ color: color }" hover-class="zx-opacity" :hover-stay-time="150"
					@tap="btnFix">确定</view>
			</view>
			<view class="zx-date-header" :style="{ backgroundColor: unitBackground }" v-if="unitTop">
				<view class="zx-date-unit" v-if="type < 4 || type == 7 || type==8">年</view>
				<view class="zx-date-unit" v-if="type < 4 || type == 7 || type==8">月</view>
				<view class="zx-date-unit" v-if="type == 1 || type == 2 || type == 7 || type==8">日</view>
				<view class="zx-date-unit" v-if="type == 1 || type == 4 || type == 5 || type == 7 || type==8">时</view>
				<view class="zx-date-unit" v-if="(type == 1 || type > 3) && type!=8">分</view>
				<view class="zx-date-unit" v-if="type > 4 && type !=8">秒</view>
			</view>
			<view @touchstart.stop="pickerstart" class="zx-date__picker-body"
				:style="{ backgroundColor: bodyBackground,height:height+'rpx' }">
				<picker-view :value="value" @change="change" class="zx-picker-view">
					<picker-view-column v-if="!reset && (type < 4 || type == 7 || type==8)">
						<view class="zx-date__column-item" :class="{ 'zx-font-size_32': !unitTop && type == 7 }"
							v-for="(item, index) in years" :key="index">
							{{ item }}
							<text class="zx-date__unit-text" v-if="!unitTop">年</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && (type < 4 || type == 7 || type==8)">
						<view class="zx-date__column-item" :class="{ 'zx-font-size_32': !unitTop && type == 7 }"
							...
							{{ formatNum(item) }}
							<text class="zx-date__unit-text" v-if="!unitTop">月</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && (type == 1 || type == 2 || type == 7 || type==8)">
						<view class="zx-date__column-item" :class="{ 'zx-font-size_32': !unitTop && type == 7 }"
							...
							{{ formatNum(item) }}
							<text class="zx-date__unit-text" v-if="!unitTop">日</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && (type == 1 || type == 4 || type == 5 || type == 7 || type==8)">
						<view class="zx-date__column-item" :class="{ 'zx-font-size_32': !unitTop && type == 7 }"
							...
							{{ formatNum(item) }}
							<text class="zx-date__unit-text" v-if="!unitTop">时</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && (type == 1 || type > 3)  && type!=8">
						<view class="zx-date__column-item" :class="{ 'zx-font-size_32': !unitTop && type == 7 }"
							...
							{{ formatNum(item) }}
							<text class="zx-date__unit-text" v-if="!unitTop">分</text>
						</view>
					</picker-view-column>
					<picker-view-column v-if="!reset && type > 4 && type!=8">
						<view class="zx-date__column-item" :class="{ 'zx-font-size_32': !unitTop && type == 7 }"
							...
							{{ formatNum(item) }}
							<text class="zx-date__unit-text" v-if="!unitTop">秒</text>
						</view>
					</picker-view-column>
				</picker-view>
			</view>
...
<style scoped>
	...
	.zx-date__picker-body {
		width: 100%;
		/* height: 520rpx; */
		overflow: hidden;
	}

	.zx-date__column-item {
		display: flex;
		align-items: center;
