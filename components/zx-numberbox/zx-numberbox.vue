<template>
	<view class="zx-numberbox">
		<view class="zx-num__icon__box" :style="{background:iconBgColor,borderRadius:radius}" @tap="reduce"
			:class="[disabled || min >= inputValue ? 'zx-disabled' : '']">
			<text class="zx-numbox-icon zx-num__icon-reduce"
				:style="{ color: iconColor, fontSize: iconSize + 'rpx',lineHeight:iconSize + 'rpx' }"></text>
		</view>
		<input type="number" v-model="inputValue" :disabled="disabled" @blur="blur" class="zx-num-input"
			:style="{ color: color, fontSize: size + 'rpx', background: backgroundColor, height: height + 'rpx', minHeight: height + 'rpx', width: width + 'rpx' }" />
		<view class="zx-num__icon__box" :style="{background:iconBgColor,borderRadius:radius}" @tap="plus"
			:class="[disabled || inputValue >= max ? 'zx-disabled' : '']">
			<text class="zx-numbox-icon zx-num__icon-plus"
				:style="{ color: iconColor, fontSize: iconSize + 'rpx',lineHeight:iconSize + 'rpx' }"></text>
		</view>
	</view>
</template>
...
<style scoped>
	...
	.zx-num__icon-reduce:before {
		content: '\e691';
	}

	.zx-num__icon-plus:before {
		content: '\e605';
	}
