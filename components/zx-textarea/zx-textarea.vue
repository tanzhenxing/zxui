<template>
	<view class="zx-textarea" :class="textareaClass" :style="[textareaStyle]">
		...
		></textarea>
		<text v-if="count" class="count" :style="{'background-color': disabled ? 'transparent' : '#fff'}">
...
export default {
	...
	computed: {
		...
		textareaClass() {
			...
			border === 'surround' && (classes = classes.concat(['zx-border', 'radius']));
			border === 'bottom' && (classes = classes.concat(['zx-border-bottom', 'no-radius']));
