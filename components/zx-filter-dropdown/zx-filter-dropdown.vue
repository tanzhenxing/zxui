<template>
	<view class="zx-filter-dropdown">
		...
		<view class="zx-dropdown-cover" v-if="isOpened" @click="handleClose"></view>
		<view class="dropdown-popup dropdown-popup-right" v-if="isOpened" @click.stop>
			...
			<!-- date -->
			<view v-if="isDate">
				<zx-datetime-picker ref="datetimepicker" :value="dateRange" type="datetimerange" return-type="timestamp" @change="datetimechange" @maskClick="timepickerclose">
					<view></view>
				</zx-datetime-picker>
			</view>
		</view>
...
onMounted(() => {
	filterValue.value = props.filterDefaultValue;
	_copyFilters();
});
...
const canReset = computed(() => {
	...
	if (isSelect.value) {
		return checkedValues.value.length > 0;
	}
	if (isRange.value) {
		return gtValue.value.length > 0 && ltValue.value.length > 0;
	}
	if (isDate.value) {
		return dateSelect.value.length > 0;
	}
	return false;
});
const isSelect = computed(() => {
	return props.filterType === DropdownType.value.Select;
});
const isSearch = computed(() => {
	return props.filterType === DropdownType.value.Search;
});
const isRange = computed(() => {
	return props.filterType === DropdownType.value.Range;
});
const isDate = computed(() => {
	return props.filterType === DropdownType.value.Date || props.filterType === DropdownType.value.Timestamp;
});

const _copyFilters = () => {
	let dl = JSON.parse(JSON.stringify(props.filterData));
...
const resetDate = () => {
	...
	dateRange.value = [dateText + ' 0:00:00', dateText + ' 23:59:59'];
};
const onDropdown = (e) => {
	openPopup();
};
const onItemClick = (e, index) => {
	...
	checkedValues.value = checkvalues;
};
...
const handleRangeSubmit = (isReset) => {
	closePopup();
	proxy.$emit('change', {
		filterType: props.filterType,
		filter: isReset === true ? [] : [parseInt(gtValue.value), parseInt(ltValue.value)]
	});
};
const handleRangeReset = () => {
	...
	gtValue.value = '';
	ltValue.value = '';
	handleRangeSubmit(true);
};
</script>

<style lang="scss">
$zx-primary: #1890ff !default;
